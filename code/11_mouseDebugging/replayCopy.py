...
def move(x, y, w, sx, sy, sw):
    ...
    print("Moving {}.{}.{}".format(int_x,int_y,int_x))
    sx = sx + int_x
    sy = sy + int_y
    sw = sw + int_w
...
def mode0Loop():
    ...
    while True:
        ...
        if testDataRowCounter < nTestDataRows and testDataRowCounter > -1:
            mv = allMVs[testDataRowCounter]
            if directionFwd:
                testDataRowCounter = testDataRowCounter + 1
            else:
                testDataRowCounter = testDataRowCounter - 1
                mv[0] = -mv[0]
                mv[1] = -mv[1]
